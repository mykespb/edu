...
ap0_short = airports_dict[short[1]['dept']]
ap1_short = airports_dict[short[1]['dest']]

ap0_long = airports_dict[long[1]['dept']]
ap1_long = airports_dict[long[1]['dest']]

apsp0 = f"{ap0_short['name']} ({ap0_short['country']})"
apsp1 = f"{ap1_short['name']} ({ap1_short['country']})"

aplp0 = f"{ap0_long['name']} ({ap0_long['country']})"
aplp1 = f"{ap1_long['name']} ({ap1_long['country']})"

print(f"""
найдено:
самый короткий рейс: {short[0]:5} км, из {short[1]['dept']} ({apsp0}) в {short[1]['dest']} ({apsp1})
самый длинный рейс:  {long[0]:5} км, из {long[1]['dept']} ({aplp0}) в {long[1]['dest']} ({aplp1})
""")
...
print(f"""
найдено:
самый короткий рейс: {short[0]:5} минут, из {short[1]['dept']} ({apsp0}) в {short[1]['dest']} ({apsp1})
самый длинный рейс:  {long[0]:5} минут, из {long[1]['dept']} ({aplp0}) в {long[1]['dest']} ({aplp1})
""")
...
# ~ найдено:
# ~ самый короткий рейс:   210 км, из APA (Сентенниал (Денвер) (США)) в ASE (Аспен — Питкин Каунти (США))
# ~ самый длинный рейс:  15340 км, из SIN (Чанги (Сингапур) (Сингапур)) в EWR (Либерти (Ньюарк) (США))
...
# ~ найдено:
# ~ самый короткий рейс:    35 минут, из APA (Сентенниал (Денвер) (США)) в ASE (Аспен — Питкин Каунти (США))
# ~ самый длинный рейс:   1125 минут, из SIN (Чанги (Сингапур) (Сингапур)) в EWR (Либерти (Ньюарк) (США))
