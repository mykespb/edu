#!/usr/bin/env python

# Mikhail (myke) Kolodin, 2024
# 2025-01-23 2025-02-05 2.1
# chess-1948.py

# ~ Дана турнирная таблица, 
# ~ Кто стал победителем турнира и чемпионом мира по шахматам 1948 года?

# -----------------------------------------------

turnir = """
Ботвинник ННВННВВВВПВНПВВВНВНН
Керес     ППППВВВНПНПНВПНВНВВВ
Решевский ПНВППННПННВНПВНВННВВ
Смыслов   ННПННППНВНННВННВВПВВ
Эйве      ПНПННППВПППНППППННПП
"""

# -----------------------------------------------

def main1():
    """запуск"""

    print("-----------------------------------------------")
    table = []
    values = { 'В': 1, 'Н': 0.5, 'П': 0 }

    for person in turnir.strip().splitlines():
        name, games = person.strip().split()
        print(f"игрок {name:10} играл так: {games}")

        table.append( (name,
            sum (map(
                lambda x: values[x], games
                ))
            ))

    table.sort(key = lambda x: x[1], reverse = True)

    print("все участники:", *table)
    print("победитель:", table[0][0], "с", round(table[0][1], 1), "очками.")

# -----------------------------------------------

def points(s):
    return s.count('В') + s.count('Н') / 2

def main2():
    """запуск"""

    print("-----------------------------------------------")
    best_points = -1
    best_name   = ''

    for person in turnir.strip().splitlines():
        name, games = person.strip().split()

        this_points = points(games)
        print(f"игрок {name:10} играл так: {games} и набрал {this_points:6} баллов")

        if this_points > best_points:
            best_name = name
            best_points = this_points

    print("победитель:", best_name, "с", best_points, "очками.")


main1()
main2()
print("-----------------------------------------------")

# -----------------------------------------------

# ~ -----------------------------------------------
# ~ игрок Ботвинник  играл так: ННВННВВВВПВНПВВВНВНН
# ~ игрок Керес      играл так: ППППВВВНПНПНВПНВНВВВ
# ~ игрок Решевский  играл так: ПНВППННПННВНПВНВННВВ
# ~ игрок Смыслов    играл так: ННПННППНВНННВННВВПВВ
# ~ игрок Эйве       играл так: ПНПННППВПППНППППННПП
# ~ все участники: ('Ботвинник', 14.0) ('Смыслов', 11.0) ('Керес', 10.5) ('Решевский', 10.5) ('Эйве', 4.0)
# ~ победитель: Ботвинник с 14.0 очками.
# ~ -----------------------------------------------
# ~ игрок Ботвинник  играл так: ННВННВВВВПВНПВВВНВНН и набрал   14.0 баллов
# ~ игрок Керес      играл так: ППППВВВНПНПНВПНВНВВВ и набрал   10.5 баллов
# ~ игрок Решевский  играл так: ПНВППННПННВНПВНВННВВ и набрал   10.5 баллов
# ~ игрок Смыслов    играл так: ННПННППНВНННВННВВПВВ и набрал   11.0 баллов
# ~ игрок Эйве       играл так: ПНПННППВПППНППППННПП и набрал    4.0 баллов
# ~ победитель: Ботвинник с 14.0 очками.
# ~ -----------------------------------------------

# -----------------------------------------------

# ~ Дана турнирная таблица вида
# ~ (фамилии указаны по алфавиту)

# ~ ===
# ~ Ботвинник ННВННВВВВПВНПВВВНВНН
# ~ Керес     ППППВВВНПНПНВПНВНВВВ
# ~ Решевский ПНВППННПННВНПВНВННВВ
# ~ Смыслов   ННПННППНВНННВННВВПВВ
# ~ Эйве      ПНПННППВПППНППППННПП
# ~ ===

# ~ где В=выигрыш (1 очко), Н=ничья (1/2 очка), П=поражение (0 очков).
# ~ Кто стал победителем турнира и чемпионом мира по шахматам 1948 года?

# ~ См. Матч-турнир за звание чемпиона мира по шахматам 1948
# ~ https://ru.wikipedia.org/wiki/%D0%9C%D0%B0%D1%82%D1%87-%D1%82%D1%83%D1%80%D0%BD%D0%B8%D1%80_%D0%B7%D0%B0_%D0%B7%D0%B2%D0%B0%D0%BD%D0%B8%D0%B5_%D1%87%D0%B5%D0%BC%D0%BF%D0%B8%D0%BE%D0%BD%D0%B0_%D0%BC%D0%B8%D1%80%D0%B0_%D0%BF%D0%BE_%D1%88%D0%B0%D1%85%D0%BC%D0%B0%D1%82%D0%B0%D0%BC_1948
# ~ Михаил Ботвинник захватил лидерство с первых туров и выиграл турнир с заметным преимуществом, имея положительный баланс со всеми соперниками и проиграв лишь 2 партии (причём вторую — уже обеспечив себе победу в турнире). Он стал шестым (и первым советским) чемпионом мира по шахматам.

# Турнирная таблица
# №	Участники	1	2	3	4	5	+	−	=	Очки	Место
# 1	Михаил Ботвинник						½	½	1	½	½	1	1	1	1	0	1	½	0	1	1	1	½	1	½	½	10	2	8	14	1
# 2	Василий Смыслов	½	½	0	½	½						0	0	½	1	½	½	½	1	½	½	1	1	0	1	1	6	4	10	11	2
# 3	Пауль Керес	0	0	0	0	1	1	1	½	0	½						0	½	1	0	½	1	½	1	1	1	8	7	5	10½	3—4
# 4	Самуэль Решевский	0	½	1	0	0	½	½	0	½	½	1	½	0	1	½						1	½	½	1	1	6	5	9	10½	3—4
# 5	Макс Эйве	0	½	0	½	½	0	0	1	0	0	0	½	0	0	0	0	½	½	0	0						1	13	6	4	5

# -----------------------------------------------

# ~ https://www.youtube.com/watch?v=QXOipRJpg20
# ~ Фрагмент из фильма "Белый снег России" (1980), Александр Алехин вслепую одновременно играет 32 партии против немецких офицеров.

# -----------------------------------------------
