...
def test(what: str) -> bool:
    ...
    for num, txt in enumerate(what.strip().splitlines(), 1):
        ...
        # проверка
        if not tfrom.isnumeric():
            print(f"время {tfrom} в строке {num} ('{desc}') указано неправильно: это не число")
            continue

        if not tto.isnumeric():
            print(f"время {tto} в строке {num} ('{desc}') указано неправильно: это не число")
            continue

        if tfrom > '2400':
            print(f"время {tfrom} в строке {num} ('{desc}') указано неправильно: оно более длины суток")
            continue

        if tto > '2400':
            print(f"время {tto} в строке {num} ('{desc}') указано неправильно: оно более длины суток")
            continue

        if tfrom > tto:
            print(f"время {tfrom}-{tto} в строке {num} ('{desc}') указано неправильно: поменяйте данные местами")
            continue
...
# ~ время xxyy в строке 15 ('делай раз') указано неправильно: это не число
# ~ время 2599 в строке 16 ('делай два') указано неправильно: оно более длины суток
# ~ время 1300-1200 в строке 17 ('отдых') указано неправильно: поменяйте данные местами
# ~ 0800 0815 - зарядка
# ~ 0845 0900 - завтрак
# ~ 0900 0945 - математика
# ~ 0900 1600 - пение
